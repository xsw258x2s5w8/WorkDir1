                    ...
                    lastFocusedWidget(0)
{
    ...
    connect(this->inputform->pushButton_54,SIGNAL(clicked()),this,SLOT(deleteCh()));
    ...
    //db.setDatabaseName("/home/hyl/Project/WeightSensor/WeighSensor/data/data.db");
    db.setDatabaseName("/opt/sqlite/D28QT.db");
   /* if(!db.open())
    {
...
void MyInputPanel::loadDoc()
{
    ...
    QFile file(":language/keyboard.xml");
                 ...
                 if(button==19)//'&'key display
                 {
                     if(str=="num")
                     {
                       btns[i]->setIconSize(btns[i]->size()*0.8);//--------------adjust
                        btns[i]->setIcon(QIcon(":language/image/&.png"));
                 ...
                 else
                 {
                     switch(button)
                    {
                     case 50:
                         ...
                         //btns[i]->setIcon(QIcon(":language/image/up.ico"));
                         btns[i]->setIcon(QIcon(":language/img/up.png"));
                         btns[i]->setText("");
                         break;
                     case 51:
                         btns[i]->setIconSize(btns[i]->size());
                         //btns[i]->setIcon(QIcon(":language/image/left.ico"));
                         btns[i]->setIcon(QIcon(":language/img/left.png"));
                         btns[i]->setText("");
                         break;
                     case 52:
                         btns[i]->setIconSize(btns[i]->size());
                         //btns[i]->setIcon(QIcon(":language/image/right.ico"));
                         btns[i]->setIcon(QIcon(":language/img/right.png"));
                         btns[i]->setText("");
                         break;
                     case 53:
                         btns[i]->setIconSize(btns[i]->size());
                         //btns[i]->setIcon(QIcon(":language/image/down.ico"));
                         btns[i]->setIcon(QIcon(":language/img/down.png"));
                         btns[i]->setText("");
                         break;
                     case 54:
                     case 55:
...
void MyInputPanel::btnClicked(QWidget *w)
{
     ...
     {
         //当用中文输入法时，键盘删除按键就无法删除焦点文本的内容
         if(!(chn&&!num1&&btn==54&&!cap))
            emit sendInt(btn);
     }
}
...
void MyInputPanel::deleteCh()
{
    ...
    if(chn&&!num1&&!cap)
    {
        ...
        QString temp=chnpanel->chnLineEdit->text();
        temp.chop(1);
        chnpanel->chnLineEdit->setText(temp);
    }
    //当输入数字或是英文时
    else
    {
